...
describe('JpSelect', function () {
  ...
  it('should render on the page', async function () {
    const element = await this.page.waitForSelector('jp-select');
  ...
  it("should have a value of 'one'", async function () {
    const element = await this.page.waitForSelector('jp-select');
    expect(await element?.evaluate(node => (node as JpSelect).value)).to.equal(
      '1'
    );
  });
  ...
  it("should have a text content of 'option 1'", async function () {
    const element = await this.page.waitForSelector(
      'jp-select .selected-value'
    );
    expect(
      await element?.evaluate((node: HTMLElement) => node.innerText)
    ).to.equal('option 1');
  });
            ...
            it(`of type '${eventName}'`, async function () {
              ...
              await element.evaluate((node, eventName) => {
                node.addEventListener(
                  eventName,
                  async (e: CustomEvent) => await window['sendEvent'](e.type)
                );
              }, eventName);
  ...
  describe('when opened', function () {
    it('should scroll the selected option into view', async function () {
      ...
      const selectedOption = (await element.$(
        '.listbox'
      )) as ElementHandle<JpOption>;
